...
/// Diagnostic test to debug V1 UPER decoding
final class V1DiagnosticTest: XCTestCase {

    func testFullStructDecode() throws {
        let data = TestTicketsV1.allElementsData
        let ticket = try FCBVersionDecoder.decode(data: data, version: 1)
        print("Decoded OK!")
    ...
    func testDocumentByDocument() throws {
        ...
        // IssuingData
        let _ = try IssuingDataV1(from: &decoder)
        print("IssuingData OK, pos=\(decoder.position)")

        // TravelerData
        if rootPresence[0] {
            let _ = try TravelerDataV1(from: &decoder)
        ...
        if rootPresence[1] {
            ...
            for i in 0..<docCount {
                ...
                do {
                    let doc = try DocumentDataV1(from: &decoder)
    ...
    func testReservationFieldByField() throws {
        ...
        let rootPresence = try decoder.decodePresenceBitmap(count: 4)
        let issuingData = try IssuingDataV1(from: &decoder)
        print("V1 IssuingData OK, pos=\(decoder.position), issuingTime=\(issuingData.issuingTime as Any)")
        if rootPresence[0] {
            let _ = try TravelerDataV1(from: &decoder)
            print("V1 TravelerData OK, pos=\(decoder.position)")
        }
        let _ = try decoder.decodeLengthDeterminant() // doc count
        print("V1 docCount pos=\(decoder.position)")
        ...
        print("V1 reservation start pos=\(decoder.position)")

        // ReservationDataV1 - decode field by field
        let hasExtensions = try decoder.decodeBit()
        let presence = try decoder.decodePresenceBitmap(count: 43)
        print("  V1 Reservation presence bitmap: \(presence.map { $0 ? "1" : "0" }.joined())")
        print("  V1 Reservation hasExtensions=\(hasExtensions)")
        var idx = 0
        ...
        try tryField("productIdNum") { if presence[idx] { let _ = try decoder.decodeConstrainedInt(min: 0, max: 32000) }; idx += 1 }
        ...
        try tryField("service") { if presence[idx] { let _ = try ServiceTypeV1(from: &decoder) }; idx += 1 }
        try tryField("stationCodeTable") { if presence[idx] { let _ = try CodeTableTypeV1(from: &decoder) }; idx += 1 }
        ...
        try tryField("departureTime") { let _ = try decoder.decodeConstrainedInt(min: 0, max: 1440) }
        try tryField("departureUTCOffset") { if presence[idx] { let _ = try decoder.decodeConstrainedInt(min: -60, max: 60) }; idx += 1 }
        try tryField("arrivalDate") { if presence[idx] { let _ = try decoder.decodeConstrainedInt(min: 0, max: 20) }; idx += 1 }
        try tryField("arrivalTime") { if presence[idx] { let _ = try decoder.decodeConstrainedInt(min: 0, max: 1440) }; idx += 1 }
        ...
        try tryField("classCode") { if presence[idx] { let _ = try TravelClassTypeV1(from: &decoder) }; idx += 1 }
        ...
        try tryField("places") { if presence[idx] { let _ = try PlacesTypeV1(from: &decoder) }; idx += 1 }
        try tryField("additionalPlaces") { if presence[idx] { let _ = try PlacesTypeV1(from: &decoder) }; idx += 1 }
        try tryField("bicyclePlaces") { if presence[idx] { let _ = try PlacesTypeV1(from: &decoder) }; idx += 1 }
        try tryField("compartmentDetails") { if presence[idx] { let _ = try CompartmentDetailsTypeV1(from: &decoder) }; idx += 1 }
        try tryField("numberOfOverbooked") { if presence[idx] { let _ = try decoder.decodeConstrainedInt(min: 0, max: 200) }; idx += 1 }
        try tryField("berth") { if presence[idx] { let _: [BerthDetailDataV1] = try decoder.decodeSequenceOf() }; idx += 1 }
        try tryField("tariff") { if presence[idx] { let _: [TariffTypeV1] = try decoder.decodeSequenceOf() }; idx += 1 }
        try tryField("priceType") { if presence[idx] { let _ = try PriceTypeTypeV1(from: &decoder) }; idx += 1 }
        try tryField("price") { if presence[idx] { let _ = Int(try decoder.decodeUnconstrainedInteger()) }; idx += 1 }
        try tryField("vatDetail") { if presence[idx] { let _: [VatDetailTypeV1] = try decoder.decodeSequenceOf() }; idx += 1 }
        ...
        try tryField("luggage") { if presence[idx] { let _ = try LuggageRestrictionTypeV1(from: &decoder) }; idx += 1 }
        try tryField("infoText") { if presence[idx] { let _ = try decoder.decodeUTF8String() }; idx += 1 }
        try tryField("extensionData") { if presence[idx] { let _ = try ExtensionDataV1(from: &decoder) } }
        ...
        print("V1 Reservation complete, pos=\(decoder.position)")
    }
}
