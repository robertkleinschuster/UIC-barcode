...
/// Converts version-independent API objects back to V1 ASN.1 model objects.
/// This is the inverse of ASNToAPIDecoderV1.
public enum APIToASNEncoderV1 {

    /// Convert a version-independent API ticket to a V1 UicRailTicketDataV1
    static func encode(_ api: UicRailTicket) -> UicRailTicketDataV1 {
        var ticket = UicRailTicketDataV1()
    ...
    static func convertIssuingDetail(_ api: IssuingDetail) -> IssuingDataV1 {
        var asn = IssuingDataV1()
    ...
    static func convertTravelerDetail(_ api: TravelerDetail) -> TravelerDataV1 {
        var asn = TravelerDataV1()
    ...
    static func convertTraveler(_ api: Traveler) -> TravelerTypeV1 {
        var asn = TravelerTypeV1()
        ...
        asn.title = api.title
        if let g = api.gender { asn.gender = GenderTypeV1(rawValue: g.rawValue) }
        asn.ticketHolder = api.ticketHolder
        if let pt = api.passengerType { asn.passengerType = PassengerTypeV1(rawValue: pt.rawValue) }
        ...
        // V1 has yearOfBirth + dayOfBirth (day-of-year 1..366), NO monthOfBirth
        if let dob = api.dateOfBirth {
            ...
            var utcCal = cal; utcCal.timeZone = utc
            asn.dayOfBirth = utcCal.ordinality(of: .day, in: .year, for: dob)
        }

        if !api.status.isEmpty {
            asn.status = api.status.map { s in
                var cs = CustomerStatusTypeV1()
    ...
    static func convertControlDetail(_ api: ControlDetail) -> ControlDataV1 {
        var asn = ControlDataV1()
    ...
    static func convertDocument(_ api: APIDocumentData, issuingDate: Date?) -> DocumentDataV1? {
        var doc = DocumentDataV1()
        var detail = TicketDetailDataV1()
    ...
    static func convertReservation(_ api: Reservation, issuingDate: Date?) -> ReservationDataV1 {
        var asn = ReservationDataV1()

        if let ref = api.reference { splitNumIA5(ref, num: &asn.referenceNum, ia5: &asn.referenceIA5) }
        if let pid = api.productId { splitNumIA5Constrained(pid, max: 32000, num: &asn.productIdNum, ia5: &asn.productIdIA5) }
        ...
        if let ct = api.stationCodeTable { asn.stationCodeTable = CodeTableTypeV1(rawValue: ct.rawValue) }
        ...
        if let cc = api.classCode { asn.classCode = TravelClassTypeV1(rawValue: cc.rawValue) }
        asn.serviceLevel = api.serviceLevel
        if let svc = api.service { asn.service = ServiceTypeV1(rawValue: svc.rawValue) }
        ...
        asn.infoText = api.infoText
        if let pt = api.priceType { asn.priceType = PriceTypeTypeV1(rawValue: pt.rawValue) }
        asn.price = api.price
    ...
    static func convertOpenTicket(_ api: OpenTicket, issuingDate: Date?) -> OpenTicketDataV1 {
        var asn = OpenTicketDataV1()
        ...
        // V1 does not have includedTransportTypes/excludedTransportTypes
    ...
    static func convertPass(_ api: Pass, issuingDate: Date?) -> PassDataV1 {
        var asn = PassDataV1()
        ...
        // V1 does not have trainValidity
    ...
    static func convertCarCarriageReservation(_ api: CarCarriageReservationAPI, issuingDate: Date?) -> CarCarriageReservationDataV1 {
        var asn = CarCarriageReservationDataV1()
        ...
        if let rrt = api.roofRackType { asn.roofRackType = RoofRackTypeV1(rawValue: rrt.rawValue) }
        if let ld = api.loadingDeck { asn.loadingDeck = LoadingDeckTypeV1(rawValue: ld.rawValue) }
        ...
        // V1 tariff is a single non-optional TariffTypeV1, use first tariff from API
        if let firstTariff = api.tariffs.first { asn.tariff = convertTariff(firstTariff) }
        if !api.vatDetails.isEmpty { asn.vatDetail = api.vatDetails.map { convertVatDetail($0) } }
        if let ext = api.extensionData { asn.extensionData = convertExtension(ext) }
    ...
    static func convertCustomerCard(_ api: CustomerCardAPI, issuingDate: Date?) -> CustomerCardDataV1 {
        var asn = CustomerCardDataV1()
    ...
    static func convertCounterMark(_ api: CounterMarkAPI, issuingDate: Date?) -> CountermarkDataV1 {
        var asn = CountermarkDataV1()
    ...
    static func convertParkingGround(_ api: ParkingGroundAPI, issuingDate: Date?) -> ParkingGroundDataV1 {
        var asn = ParkingGroundDataV1()
        ...
        // Date offsets — V1 uses untilParkingDate (not toParkingDate)
        ...
        if let toDate = api.toParkingDate, let fromDate = api.fromParkingDate {
            asn.untilParkingDate = DateTimeUtils.getDateDifference(fromDate, toDate)
        }
    ...
    static func convertFIPTicket(_ api: FIPTicketAPI, issuingDate: Date?) -> FIPTicketDataV1 {
        var asn = FIPTicketDataV1()
    ...
    static func convertStationPassage(_ api: StationPassageAPI, issuingDate: Date?) -> StationPassageDataV1 {
        var asn = StationPassageDataV1()
        ...
        // Date offsets — V1 validFromDay is mandatory (Int = 0)
        if let validFrom = api.validFrom, let issuingDate {
    ...
    static func convertDelayConfirmation(_ api: DelayConfirmationAPI, issuingDate: Date?) -> DelayConfirmationV1 {
        var asn = DelayConfirmationV1()
        ...
        asn.stationNum = api.stationNum
        asn.stationIA5 = api.stationIA5
        if let ct = api.confirmationType { asn.confirmationType = ConfirmationTypeTypeV1(rawValue: ct.rawValue) }
        asn.infoText = api.infoText
        ...
        // V1 uses departure* fields (not plannedArrival* like V3)
        if let depDate = api.plannedArrivalDate {
            let cal = Calendar(identifier: .gregorian)
            let utc = TimeZone(identifier: "UTC")!
            let components = cal.dateComponents(in: utc, from: depDate)
            asn.departureYear = components.year
            var utcCal = cal; utcCal.timeZone = utc
            asn.departureDay = utcCal.ordinality(of: .day, in: .year, for: depDate)
    ...
    static func convertVoucher(_ api: VoucherAPI, issuingDate: Date?) -> VoucherDataV1 {
        var asn = VoucherDataV1()
        ...
        // Date offsets — VoucherDataV1 uses year + day-of-year pattern
        if let validFrom = api.validFrom {
            let cal = Calendar(identifier: .gregorian)
            let utc = TimeZone(identifier: "UTC")!
            let components = cal.dateComponents(in: utc, from: validFrom)
            asn.validFromYear = components.year ?? 2016
            var utcCal = cal; utcCal.timeZone = utc
            asn.validFromDay = utcCal.ordinality(of: .day, in: .year, for: validFrom) ?? 0
        }
        if let validUntil = api.validUntil {
            let cal = Calendar(identifier: .gregorian)
            let utc = TimeZone(identifier: "UTC")!
            ...
            var utcCal = cal; utcCal.timeZone = utc
            asn.validUntilDay = utcCal.ordinality(of: .day, in: .year, for: validUntil) ?? 0
        }
    ...
    static func convertDocumentExtension(_ api: DocumentExtensionAPI) -> ExtensionDataV1 {
        var ext = ExtensionDataV1()
    ...
    static func convertToken(_ api: TokenAPI?) -> TokenTypeV1? {
    ...
    static func convertExtension(_ api: TicketExtension) -> ExtensionDataV1 {
        var ext = ExtensionDataV1()
        ext.extensionId = api.extensionId ?? ""
        ext.extensionData = api.extensionData ?? Data()
        return ext
    ...
    static func convertGeoCoordinate(_ api: GeoCoordinateAPI) -> GeoCoordinateTypeV1 {
        var g = GeoCoordinateTypeV1()
        g.longitude = Int(api.longitude)
        g.latitude = Int(api.latitude)
        g.coordinateSystem = GeoCoordinateSystemTypeV1(rawValue: api.coordinateSystem.rawValue)
        g.geoUnit = GeoUnitTypeV1(rawValue: api.accuracy.rawValue)
        g.hemisphereLongitude = HemisphereLongitudeTypeV1(rawValue: api.hemisphereLongitude.rawValue)
        g.hemisphereLatitude = HemisphereLatitudeTypeV1(rawValue: api.hemisphereLatitude.rawValue)
        return g
    }

    static func convertCardReference(_ api: CardReferenceAPI) -> CardReferenceTypeV1 {
        var c = CardReferenceTypeV1()
    ...
    static func convertTicketLink(_ api: TicketLinkAPI) -> TicketLinkTypeV1 {
        var t = TicketLinkTypeV1()
        ...
        if let tt = api.ticketType { t.ticketType = TicketTypeV1(rawValue: tt.rawValue) }
        if let lm = api.linkMode { t.linkMode = LinkModeV1(rawValue: lm.rawValue) }
        return t
    }

    static func convertPlaces(_ api: PlacesAPI) -> PlacesTypeV1 {
        var p = PlacesTypeV1()
        p.coach = api.coach
    ...
    static func convertCompartmentDetails(_ api: CompartmentDetailsAPI) -> CompartmentDetailsTypeV1 {
        var c = CompartmentDetailsTypeV1()
        ...
        if let pos = api.position { c.position = CompartmentPositionTypeV1(rawValue: pos.rawValue) }
        return c
    }

    static func convertBerth(_ api: BerthAPI) -> BerthDetailDataV1 {
        var b = BerthDetailDataV1()
        if let bt = api.berthType { b.berthType = BerthTypeTypeV1(rawValue: bt.rawValue) ?? .single }
        if api.numberOfBerths != 0 { b.numberOfBerths = api.numberOfBerths }
        if let g = api.gender { b.gender = CompartmentGenderTypeV1(rawValue: g.rawValue) }
        return b
    }

    static func convertTariff(_ api: TariffAPI) -> TariffTypeV1 {
        var t = TariffTypeV1()
        if api.numberOfPassengers != 1 { t.numberOfPassengers = api.numberOfPassengers }
        if let pt = api.passengerType { t.passengerType = PassengerTypeV1(rawValue: pt.rawValue) }
    ...
    static func convertSeriesDetail(_ api: SeriesDetailAPI) -> SeriesDetailTypeV1 {
        var s = SeriesDetailTypeV1()
    ...
    static func convertVatDetail(_ api: VatDetailAPI) -> VatDetailTypeV1 {
        var v = VatDetailTypeV1()
    ...
    static func convertLuggageRestriction(_ api: LuggageRestrictionAPI) -> LuggageRestrictionTypeV1 {
        var l = LuggageRestrictionTypeV1()
    ...
    static func convertRegionalValidity(_ api: RegionalValidityAPI) -> RegionalValidityTypeV1? {
        var r = RegionalValidityTypeV1()
        switch api {
    ...
    static func convertTrainLink(_ api: TrainLinkAPI) -> TrainLinkTypeV1 {
        var t = TrainLinkTypeV1()
    ...
    static func convertViaStation(_ api: ViaStationAPI) -> ViaStationTypeV1 {
        var v = ViaStationTypeV1()
        if let ct = api.stationCodeTable { v.stationCodeTable = CodeTableTypeV1(rawValue: ct.rawValue) }
    ...
    static func convertZone(_ api: ZoneAPI) -> ZoneTypeV1 {
        var z = ZoneTypeV1()
        if let ct = api.stationCodeTable { z.stationCodeTable = CodeTableTypeV1(rawValue: ct.rawValue) }
    ...
    static func convertLine(_ api: LineAPI) -> LineTypeV1 {
        var l = LineTypeV1()
        if let ct = api.stationCodeTable { l.stationCodeTable = CodeTableTypeV1(rawValue: ct.rawValue) }
    ...
    static func convertPolygone(_ api: PolygoneAPI) -> PolygoneTypeV1 {
        ...
        let edges = api.edges.map { edge in
            var d = DeltaCoordinatesV1()
            d.longitude = edge.longitude
            d.latitude = edge.latitude
            return d
        }
        return PolygoneTypeV1(firstEdge: firstEdge, edges: edges)
    }

    static func convertReturnRouteDescription(_ api: ReturnRouteDescriptionAPI) -> ReturnRouteDescriptionTypeV1 {
        var r = ReturnRouteDescriptionTypeV1()
    ...
    static func convertIncludedOpenTicket(_ api: IncludedOpenTicketAPI) -> IncludedOpenTicketTypeV1 {
        var i = IncludedOpenTicketTypeV1()
        ...
        if let ct = api.stationCodeTable { i.stationCodeTable = CodeTableTypeV1(rawValue: ct.rawValue) }
        if let cc = api.classCode { i.classCode = TravelClassTypeV1(rawValue: cc.rawValue) }
        ...
        // V1 IncludedOpenTicketType does not have includedTransportTypes/excludedTransportTypes
    ...
    static func convertValidityDetails(_ api: ValidityDetailsAPI) -> ValidityPeriodDetailTypeV1 {
        var vpd = ValidityPeriodDetailTypeV1()
        if !api.validityPeriods.isEmpty {
            vpd.validityPeriod = api.validityPeriods.map { _ in
                ValidityPeriodTypeV1()
            }
        }
        if !api.excludedTimeRanges.isEmpty {
            vpd.excludedTimeRange = api.excludedTimeRanges.map { range in
                TimeRangeTypeV1(
                    fromTime: range.fromTime ?? 0,
                    untilTime: range.untilTime ?? 0
                )
            }
        }
        return vpd
