...
public struct CompartmentDetailsType: ASN1Decodable {
    ...
    public init(from decoder: inout UPERDecoder) throws {
        ...
        // 7 fields with presence bits (@Asn1Optional or @Asn1Default)
        ...
        if presence[6] {
            let posValue = try decoder.decodeEnumerated(rootCount: 3)
            position = CompartmentPositionType(rawValue: posValue)
        } else {
...
extension CompartmentDetailsType: ASN1Encodable {
    public func encode(to encoder: inout UPEREncoder) throws {
        ...
        if positionPresent { try encoder.encodeEnumerated(position!.rawValue, rootCount: 3) }
    }
}
