...
struct CarCarriageReservationDataV1: ASN1Decodable {
    var trainNum: Int?
    var trainIA5: String?
    ...
    init(from decoder: inout UPERDecoder) throws {
        ...
        if presence[idx] { beginLoadingTime = try decoder.decodeConstrainedInt(min: 0, max: 1440) }; idx += 1
        if presence[idx] { endLoadingTime = try decoder.decodeConstrainedInt(min: 0, max: 1440) }; idx += 1
        ...
        if presence[idx] {
            roofRackType = try RoofRackTypeV1(from: &decoder)
        } else {
            roofRackType = .norack
        }; idx += 1
        ...
        if presence[idx] {
            loadingDeck = try LoadingDeckTypeV1(from: &decoder)
        } else {
            loadingDeck = .upper
        }; idx += 1
        if presence[idx] {
        ...
        // tariff is MANDATORY
        tariff = try TariffTypeV1(from: &decoder)
...
// MARK: - CarCarriageReservationDataV1 Encoding

extension CarCarriageReservationDataV1: ASN1Encodable {
    func encode(to encoder: inout UPEREncoder) throws {
        ...
        if let v = beginLoadingTime { try encoder.encodeConstrainedInt(v, min: 0, max: 1440) }
        if let v = endLoadingTime { try encoder.encodeConstrainedInt(v, min: 0, max: 1440) }
        ...
        if roofRackTypePresent { try encoder.encodeEnumerated(roofRackType!.rawValue, rootCount: RoofRackTypeV1.rootValueCount, hasExtensionMarker: RoofRackTypeV1.hasExtensionMarker) }
        ...
        if loadingDeckPresent { try encoder.encodeEnumerated(loadingDeck!.rawValue, rootCount: LoadingDeckTypeV1.rootValueCount) }
