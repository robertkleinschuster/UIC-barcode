...
struct DocumentDataV1: ASN1Decodable {
    static let hasExtensionMarker = true
    static let optionalFieldCount = 1

    var token: TokenTypeV1?
    var ticket: TicketDetailDataV1

    init() {
        self.ticket = TicketDetailDataV1()
    }
    ...
    init(from decoder: inout UPERDecoder) throws {
        ...
        if presence[0] { token = try TokenTypeV1(from: &decoder) }

        ticket = try TicketDetailDataV1(from: &decoder)
...
extension DocumentDataV1: ASN1Encodable {
