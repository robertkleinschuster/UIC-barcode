...
struct TravelerTypeV1: ASN1Decodable {
    static let hasExtensionMarker = true
    static let optionalFieldCount = 17
    ...
    var status: [CustomerStatusTypeV1]?
    ...
    init(from decoder: inout UPERDecoder) throws {
        ...
        if presence[idx] { gender = try GenderTypeV1(from: &decoder) }; idx += 1
        ...
        if presence[idx] { dayOfBirth = try decoder.decodeConstrainedInt(min: 0, max: 366) }; idx += 1
...
extension TravelerTypeV1: ASN1Encodable {
    func encode(to encoder: inout UPEREncoder) throws {
        ...
        if let v = gender { try encoder.encodeEnumerated(v.rawValue, rootCount: GenderTypeV1.rootValueCount, hasExtensionMarker: GenderTypeV1.hasExtensionMarker) }
        ...
        if let v = dayOfBirth { try encoder.encodeConstrainedInt(v, min: 0, max: 366) }
