...
struct PassDataV2: ASN1Decodable {
    ...
    var validityPeriodDetails: ValidityPeriodDetailTypeV2?
    ...
    init(from decoder: inout UPERDecoder) throws {
        ...
        // 33 optional+default fields (classCode(D), validFromDay(D), validUntilDay(D) + 28 optional + 2 new UTCOffset)
        // V1 had 31; V2 adds validFromUTCOffset and validUntilUTCOffset = +2
        ...
        if presence[idx] { validityPeriodDetails = try ValidityPeriodDetailTypeV2(from: &decoder) }; idx += 1
...
// MARK: - PassDataV2 Encoding

extension PassDataV2: ASN1Encodable {
    func encode(to encoder: inout UPEREncoder) throws {
        ...
        // V2: 33 optional+default fields; no mandatory fields
        ...
        if let v = countries {
            try encoder.encodeLengthDeterminant(v.count)
            for c in v { try encoder.encodeConstrainedInt(c, min: 1, max: 250) }
        }
        if let v = includedCarrierNum {
            try encoder.encodeLengthDeterminant(v.count)
            for num in v { try encoder.encodeConstrainedInt(num, min: 1, max: 32000) }
        }
        if let v = includedCarrierIA5 {
            try encoder.encodeLengthDeterminant(v.count)
            for s in v { try encoder.encodeIA5String(s) }
        }
        if let v = excludedCarrierNum {
            try encoder.encodeLengthDeterminant(v.count)
            for num in v { try encoder.encodeConstrainedInt(num, min: 1, max: 32000) }
        }
        if let v = excludedCarrierIA5 {
            try encoder.encodeLengthDeterminant(v.count)
            for s in v { try encoder.encodeIA5String(s) }
        }
