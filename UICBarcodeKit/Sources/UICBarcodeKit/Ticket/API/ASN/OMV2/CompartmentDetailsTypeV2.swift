...
struct CompartmentDetailsTypeV2: ASN1Decodable {
    var coachType: Int?
    ...
    var position: CompartmentPositionTypeV2?
    ...
    init(from decoder: inout UPERDecoder) throws {
        ...
        if presence[6] {
            position = try CompartmentPositionTypeV2(from: &decoder)
        } else {
...
extension CompartmentDetailsTypeV2: ASN1Encodable {
    func encode(to encoder: inout UPEREncoder) throws {
        ...
        if positionPresent { try encoder.encodeEnumerated(position!.rawValue, rootCount: CompartmentPositionTypeV2.rootValueCount) }
    }
}
